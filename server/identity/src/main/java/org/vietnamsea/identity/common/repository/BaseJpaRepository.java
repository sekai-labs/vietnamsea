package org.vietnamsea.identity.common.repository;
...
public interface BaseJpaRepository<T, ID extends Serializable>
  ...
  private Predicate buildComparisonPredicate(
    ...
    try {
      if (fieldType.equals(Boolean.class)) {
        ...
        return op.build(cb, root.get(field),
        ...
        if (dateStr.contains(".")) {
          ...
          formatter = DateTimeFormatter
              .ofPattern("yyyy-MM-dd HH:mm:ss" + (len == 3 ? ".SSS" : len == 6 ? ".SSSSSS" : ""));
        } else {
  ...
  default Page<T> query(Map<String, QueryFieldWrapper> param, Pageable pageable,
      Function<Map<String, QueryFieldWrapper>, Specification<T>> query) {
  ...
  default <D extends List<?>> PaginationWrapper<D> query(Map<String, QueryFieldWrapper> param, Pageable pageable,
      Function<Map<String, QueryFieldWrapper>, Specification<T>> query,
      Function<Page<T>, PaginationWrapper<D>> mapper) {
    var entityResult = findAll(query.apply(param), pageable);
    return mapper.apply(entityResult);
  }

  default <D extends List<?>> PaginationWrapper<D> query(QueryWrapper queryWrapper,
      Function<Map<String, QueryFieldWrapper>, Specification<T>> query,
      Function<Page<T>, PaginationWrapper<D>> mapper) {
